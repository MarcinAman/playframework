...
trait EvolutionsApi {
   ...
   * @return evolution scripts
   */
  def scripts(db: String, evolutions: Seq[Evolution], schema: String, table: String): Seq[Script]
   ...
   * @return evolution scripts
   */
  def scripts(db: String, reader: EvolutionsReader, schema: String, table: String): Seq[Script]
   ...
   * @return evolution scripts
   */
  def resetScripts(db: String, schema: String, table: String): Seq[Script]
   ...
   * @param table Table to keep evolutions' data
   */
  def evolve(db: String, scripts: Seq[Script], autocommit: Boolean, schema: String, table: String): Unit
   ...
   * @param table Table to keep evolutions' data
   */
  def resolve(db: String, revision: Int, schema: String, table: String): Unit

  /**
   * Apply pending evolutions for the given database.
   */
  def applyFor(dbName: String, path: File = new File("."), autocommit: Boolean = true, schema: String = "", table: String = "play_evolutions"): Unit = {
    val scripts = this.scripts(dbName, new EnvironmentEvolutionsReader(Environment.simple(path = path)), schema, table)
...
class DefaultEvolutionsApi @Inject() (dbApi: DBApi) extends EvolutionsApi {
  private def databaseEvolutions(name: String, schema: String, table: String = "play_evolutions") = new DatabaseEvolutions(dbApi.database(name), schema, table)

  def scripts(db: String, evolutions: Seq[Evolution], schema: String) = databaseEvolutions(db, schema).scripts(evolutions)

  def scripts(db: String, evolutions: Seq[Evolution], schema: String, table: String) = databaseEvolutions(db, schema, table).scripts(evolutions)

  def scripts(db: String, reader: EvolutionsReader, schema: String) = databaseEvolutions(db, schema).scripts(reader)

  def scripts(db: String, reader: EvolutionsReader, schema: String, table: String) = databaseEvolutions(db, schema, table).scripts(reader)

  def resetScripts(db: String, schema: String) = databaseEvolutions(db, schema).resetScripts()

  def resetScripts(db: String, schema: String, table: String) = databaseEvolutions(db, schema, table).resetScripts()
  ...
  def evolve(db: String, scripts: Seq[Script], autocommit: Boolean, schema: String, table: String) =
    databaseEvolutions(db, schema, table).evolve(scripts, autocommit)

  def resolve(db: String, revision: Int, schema: String) = databaseEvolutions(db, schema).resolve(revision)

  def resolve(db: String, revision: Int, schema: String, table: String) = databaseEvolutions(db, schema, table).resolve(revision)
}

/**
 * Evolutions for a particular database.
 */
class DatabaseEvolutions(database: Database, schema: String = "", tableName: String = "play_evolutions") {
  ...
  def resolve(revision: Int): Unit = {
    implicit val connection = database.getConnection(autocommit = true)
    try {
      execute("update ${schema}${evolutions_table} set state = 'applied' where state = 'applying_up' and id = " + revision)
  ...
  // SQL helpers
  import EvolutionsSchemaMapper._

  private def executeQuery[T](sql: String)(f: ResultSet => T)(implicit c: Connection): T = {
    val ps = c.createStatement
    try {
      val rs = ps.executeQuery(applySchemaAndTable(sql, schema = schema, table = tableName))
      f(rs)
    } finally {
      ps.close()
    }
  ...
  private def execute(sql: String)(implicit c: Connection): Boolean = {
    ...
    try {
      s.execute(applySchemaAndTable(sql, schema = schema, table = tableName))
    } finally {
      s.close()
    }
  }
  ...
  private def prepareAndExecute(sql: String)(block: PreparedStatement => Unit)(implicit c: Connection): Boolean = {
    val ps = c.prepareStatement(applySchemaAndTable(sql, schema = schema, table = tableName))
    try {
      block(ps)
      ps.execute()
    } finally {
